...
export async function addLink (context, options: {
  ...
  if (options.parentCid) {
    ...
    if(map.has(parentCid.toString())) {
      const block = map.get(parentCid.toString())
      parent = dagPB.decode(block)
    } else {
      const block = await context.block.get(parentCid)
      parent = dagPB.decode(block)
      map.set(parentCid.toString(), block)
    }
  }
...
const addToShardedDirectory = async (context, options) => {
  ...
  // console.log(`Lag A2 ${new Date().getTime() - date.getTime()}ms ${trace}`, )
  ...
  let node 
  if(map.has((result as any).cid.toString())) {
    const block = map.get((result as any).cid.toString())
    node = dagPB.decode(block)
  } else {
    const block = await context.block.get((result as any).cid)
    node = dagPB.decode(block)
    map.set((result as any).cid, block)
  }
  ...
  // console.log(`Lag C ${new Date().getTime() - date.getTime()}ms ${trace}`, )
  ...
  const out = await updateHamtDirectory(context, parentLinks, path[0].bucket, options)
  // console.log(`Lag D ${new Date().getTime() - date.getTime()}ms ${trace}`, )
...
const addFileToShardedDirectory = async (context, options) => {
  ...
  const path = toBucketPath(position) as any
  ...
  while (index < path.length) {
    ...
    let subShard
    if(map.has(link.Hash)) {
      const block = map.get(link.Hash)
      subShard = dagPB.decode(block)
    } else {
      const block = await context.block.get(link.Hash)
      subShard = dagPB.decode(block)
      map.set(link.Hash, block)
    }
