...
export function createTree(alg, leaves) {
...
export function createRoot(alg, leaves) {
  // assert(alg && typeof alg.root === 'function');
  // assert(Array.isArray(leaves));

  const [nodes, malleated] = createTree(alg, leaves);
  const root = nodes[(nodes as any).length - 1];
...
export function createBranch(alg, index, leaves) {
  assert(alg && typeof alg.root === 'function');
...
export function deriveRoot(alg, hash, branch, index) {
  assert(alg && typeof alg.root === 'function');
