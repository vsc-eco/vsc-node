...
enum PUBSUB_CHANNELS  {
    multicast = '/vsc/multicast/v2',
    routesAnnounce = '/vsc/multicast/v2',
...
export class PeerChannel {
    ...
    async register(id: string, handler: MessageHandleFn, options?:{loopbackOk?: boolean}) {
        this._handles[id] = {
            id,
            handler,
...
export class P2PService {
    ...
    private async handleMulticast(msg) {
        ...
        try {
            ...
            if(json_payload.type === MESSAGE_TYPES.announceNode) {
                ...
                const did_proof = await verifyMultiJWS(json_payload.payload.did_proof, this.self.oldService.identity)
    ...
    async announceNode() {
        try {
            ...
            const signers = [this.self.oldService.identity]
            ...
            const msg = {
                ...
                ts,
                node_did: this.self.oldService.identity.id,
                payload: {
