...
async function run() {
  ...
  switch (cmd) {
    ...
    case 'bin2diag': {
      ...
      const bin =
        argv.length < 4 ? await fromStdin() : new TextEncoder().encode(argv[3])
    ...
    case 'bin2hex': {
      ...
      const bin =
        process.argv.length < 4
          ? await fromStdin()
          : new TextEncoder().encode(process.argv[3])
    ...
    case 'bin2json': {
      ...
      const bin =
        argv.length < 4 ? await fromStdin() : new TextEncoder().encode(argv[3])
      return console.log(
        JSON.stringify(decode(bin), undefined, pretty ? 2 : undefined),
      )
    }
    ...
    case 'diag2json': {
      ...
      /* c8 ignore next 1 */
      const bin = fromDiag(
        argv.length < 4 ? (await fromStdin()).toString() : argv[3],
      )
      return console.log(
        JSON.stringify(decode(bin), undefined, pretty ? 2 : undefined),
      )
